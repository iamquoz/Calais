...
namespace Calais.Core
{
    internal sealed class CustomMethodInvoker
    {
        ...
        public bool TryApplyFilter<TEntity>(
            ...
            out IQueryable<TEntity> result) where TEntity : class
        {
            result = source;
            ...
            if (compatible.Count != 1)
                return HandleInvalidCustomMethod(descriptor.Field!, "filter", compatible.Count, candidates.Count);

            var context = new CalaisFilterContext(descriptor, _services);
            result = (IQueryable<TEntity>)compatible[0].Method!.Invoke(compatible[0].Target, [source, context])!;
            return true;
        }
            ...
            out IQueryable<TEntity> result) where TEntity : class
        {
            ...
            if (compatible.Count != 1)
                return HandleInvalidCustomMethod(descriptor.Field, "sort", compatible.Count, candidates.Count);
        ...
        public bool HasCustomFilter<TEntity>(FilterDescriptor descriptor, out bool invalid) where TEntity : class
        {
            invalid = false;
            if (string.IsNullOrWhiteSpace(descriptor.Field))
                return false;
        ...
        private bool HandleInvalidCustomMethod(string field, string kind, int compatibleCount, int candidateCount)
        {
            ...
            string field)
        {
            ...
            foreach (var methodService in methodServices)
            {
                ...
                var methods = methodService.GetType().GetMethods(BindingFlags.Instance | BindingFlags.Public)
                    .Where(method => string.Equals(method.Name, field, StringComparison.OrdinalIgnoreCase));

                candidates.AddRange(methods.Select(method => new CustomMethodCandidate(methodService, method)));
            }
        ...
        private static CustomMethodCandidate CloseAndValidate<TEntity>(
            ...
            Type contextType) where TEntity : class
        {
            ...
            return candidate with { Method = method };
        }

        private static MethodInfo? TryCloseGenericMethod<TEntity>(MethodInfo method) where TEntity : class
        {
